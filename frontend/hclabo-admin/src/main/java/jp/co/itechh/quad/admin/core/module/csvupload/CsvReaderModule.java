...
package jp.co.itechh.quad.admin.core.module.csvupload;
...
public class CsvReaderModule {
    ...
    private static final String INVALID_VAL_EXCEPTION_MSG_CD = "CSV-UPLOAD-004-E";
        ...
        for (Map.Entry<String, Field> entry : this.csvDtoClassFieldMap.entrySet()) {
            ...
            if (!StringUtils.isEmpty(fieldValueStr) && !StringUtils.isEmpty(fieldAnnotation.dateFormat())) {
                ...
                try {
                    ...
                    LOGGER.error("例外処理が発生しました", e);
                    // 日付への変換ができなかった場合
                    addErrorToCsvValidationResult(
                                    csvValidationResult, "HDateValidator.NOT_DATE_detail", recordCount, fieldNameJp);
                    return null;
                }
            }
            ...
            if (!StringUtils.isEmpty(fieldAnnotation.enumOutputType())) {
                ...
                //                } else {
                //                    // HTYPEへの変換ができなかった場合
                //                    addErrorToCsvValidationResult(csvValidationResult, "HItemsValidator.INVALID_detail", recordCount, fieldNameJp);
                //                    return null;
                //                }
                // HTYPEへの変換ができなかった場合は、nullのままを設定する

                if (StringUtils.isNotEmpty(fieldValueStr) && enumType == null) {
                    // HTYPEへの変換ができなかった場合
                    addInvalidErrorToCsvValidationResult(
                                    csvValidationResult, INVALID_VAL_EXCEPTION_MSG_CD, fieldNameJp, recordCount);
                    return null;
                }
     ...
     * 検証NG詳細リストにエラー内容を追加する
     *
     * @param csvValidationResult CsvUploadResult
     * @param msgId メッセージID
     * @param recordCount Csvデータ件数
     * @param args メッセージ引数
     */
    private void addInvalidErrorToCsvValidationResult(CsvValidationResult csvValidationResult,
                                                      String msgId,
                                                      String args,
                                                      int recordCount) {
        // エラーメッセージを取得する
        String message = messageSource.getMessage(msgId, new String[] {args}, Locale.getDefault());
        // 検証NG詳細リストにエラー内容を追加する
        csvValidationResult.getDetailList().add(new InvalidDetail(recordCount, EMPTY_STRING, message));
    }
     ...
     * @return バリデーション結果
     */
    public <T> CsvValidationResult validateCSV(File csvFile,
                                               List<Integer> counts,
                                               int maximumErrorDetection,
                                               List<T> csvDtoList,
                                               Class<?> csvDtoClass) {
        ...
        try {
            ...
            csvDtoList.addAll((List<T>) this.readCsv(csvFile, csvDtoClass, csvUploadResult, csvReaderOptionDto));

        } catch (Exception e) {
            LOGGER.error("例外処理が発生しました", e);
            // CSV読み込みで有り得ない例外が発生した場合
            CsvValidationResult csvValidationResult = new CsvValidationResult();
            this.createUnexpectedExceptionMsg(csvValidationResult);
