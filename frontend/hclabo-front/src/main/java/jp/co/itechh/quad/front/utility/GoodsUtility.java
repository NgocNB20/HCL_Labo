...
public class GoodsUtility {
     ...
     * @param previewDate プレビュー日付（日時＋時間）
     */
    public void createAllUnitMap(List<GoodsDto> goodsDtoList,
        ...
        for (GoodsDto goodsDto : goodsDtoList) {
            ...
            // 販売チェック
            // 購入可能なもののみ、プルダウンにセット
            if (ObjectUtils.isEmpty(previewDate)) {
                ...
                // プレビュー日時を加味した販売チェック
                if (!goodsUtility.isGoodsSales(entity.getSaleStatusPC(), entity.getSaleStartTimePC(),
                                               entity.getSaleEndTimePC(), previewDate
                                              )) {
                    continue;
                }
            }
