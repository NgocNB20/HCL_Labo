 ...
 */
@RestController
public class ClearController implements PromotionApi {
     ...
     * @param deleteOrderItemUseCase  注文商品削除ユースケース
     */
    @Autowired
    ...
    /**
     * DELETE /promotion/cancellation : 不要注文票取消
     * 不要注文票取消
     *
     * @param unnecessaryOrderSlipCancellationRequest 不要注文票取消リクエスト (required)
    ...
    public ResponseEntity<Void> delete(
                    @NotNull @Valid UnnecessaryOrderSlipCancellationRequest unnecessaryOrderSlipCancellationRequest) {

        // 不要データ削除処理
        deleteOrderItemUseCase.deleteUnnecessaryByTransactionId(
                        unnecessaryOrderSlipCancellationRequest.getTransactionId());
