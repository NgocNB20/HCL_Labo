...
public class ShipmentRegistProcessor {
    ...
    /** ロガー */
    private static final Logger LOGGER = LoggerFactory.getLogger(ShipmentRegistProcessor.class);
     ...
     */
    public void processor(ShipmentRegistRequest shipmentRegistRequest) {
        try {
            ...
            valueMap.put("BATCH_NAME", HTypeBatchName.BATCH_SHIPMENT_REGIST.getLabel());
            valueMap.put("SHOP_SEQ", "1001");
            if (shipmentRegistRequest.getErrorMailMessage() == null
                || shipmentRegistRequest.getErrorMailMessage().length() == 0) {
                valueMap.put("LIST", "");
            } else {
                valueMap.put("LIST", shipmentRegistRequest.getErrorMailMessage());
            }
            ...
            mailDto.setMailTemplateType(HTypeMailTemplateType.SHIPMENT_REGIST_ADMINISTRATOR_ERROR_MAIL);
            mailDto.setFrom(PropertiesUtil.getSystemPropertiesValue("mail.setting.shipment.regist.mail.from"));
            mailDto.setToList(Arrays.asList(PropertiesUtil.getSystemPropertiesValue(
                            "mail.setting.shipment.regist.mail.receivers").split(COMMA_DELIMITER)));
            mailDto.setSubject("【" + systemName + "】" + HTypeBatchName.BATCH_SHIPMENT_REGIST.getLabel() + "報告");
