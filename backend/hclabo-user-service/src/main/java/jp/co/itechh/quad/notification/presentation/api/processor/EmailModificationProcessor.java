...
@Component
@Scope("prototype")
public class EmailModificationProcessor {

    /** ロガー */
    private static final Logger LOGGER = LoggerFactory.getLogger(EmailModificationProcessor.class);
     ...
     */
    public void processor(EmailModificationRequest emailModificationRequest) {
        LOGGER.info("【Subscribe】ルーティングキー： user-email-modification-routing");

        // 確認メール情報取得
        ConfirmMailDao confirmMailDao = ApplicationContextUtility.getBean(ConfirmMailDao.class);
        ConfirmMailEntity confirmMailEntity = confirmMailDao.getEntity(emailModificationRequest.getConfirmMailSeq());
        ...
        MailTemplateEntity entity = mailTemplateGetLogic.execute(1001, HTypeMailTemplateType.EMAIL_MODIFICATION);
